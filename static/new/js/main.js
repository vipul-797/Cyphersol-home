...
function validateForm(event) {
  ...
  if (password !== "correct-password") {
    ...
    var loginButton = document.querySelector(".shake-button");
    loginButton.classList.add("shake-button");
    setTimeout(function() {
      loginButton.classList.remove("shake-button");
    }, 500);
  }
}
    ...
    function refreshCaptcha() {
      ...
      // Update the captcha preview
      var preview = document.querySelector('.preview');
      preview.textContent = captcha;

      // Update the hidden input field for the captcha
...
function toggleOTPInput(checkbox) {
  var otpInputContainer = document.getElementById("otpInputContainer");
  otpInputContainer.style.display = checkbox.checked ? "block" : "none";
}      